...
function addTempNotification($request,$response,$args){
    ...
    $this->notificationTempToken($user['token'],$user['source']);
}
...
function addUserNotification($request,$response,$args){
    ...
    $this->notificationRegister($user['token'],$user['user_id'],$user['source'],$user['refresh_token']); 
}
            ...
            if ($o['order_id'] == $key) {
                if (sizeof($extras)>0) {
                    foreach ($p as $p1) {
                        ...
                        foreach ($p as $p2) {
                            foreach ($extras as $e) {
                                if ($p2["order_item_id"] == $e["order_item_id"]) {
                                    $p2["extra"][] = $e;
                                }
                            }
                            array_push($productExtras, $p2);
                        }
                    }
                    $o['order_item'] = $productExtras;
                    array_push($requestArray, $o);
                }else{
                    $o['order_item'] = $p;
                    array_push($requestArray, $o);
                }
...
function refreshAccessToken($request, $response){
    ...
    if ($user) {
        $accessTtoken = $this->newAccessToken($user->user_id, $user->company_id, $user->employee_company, $user->employee_id, $jwt);        
...
function newAccessToken($userId, $companyId, $employeeCompany, $employeeId, $jwt) {
    ...
    $sth = $this->app->db->prepare("UPDATE token SET token.refresh_date = now() WHERE token.token_id = :jwt");
...
function addRefreshToken($userId) {
    ...
    try{
        ...
        /*
        $sth2 = $this->app->db->prepare("DELETE FROM token WHERE user_id = :userId");
...
function addCompany($request, $response, $args){
    ...
    foreach ($inputCompany as $key => $VALUES) {
        if($key == "user_id"){
            $userId = $VALUES;
        }else if($key != "is_selected"){//todo - is_select é para corrigir um bug do app v1.2.0, tirar futuramente
            $keys[] = $key;
            $sets[] = ":".$key;
        }
    }
    ...
    if($company){
        ...
        try{
                ...
                foreach ($inputCompany as $key => $value) {
                    ...
                    }else if ($key != "user_id" && $key != "is_selected"){//todo - is_select é para corrigir um bug do app v1.2.0, tirar futuramente
                        $sth ->bindValue(':'.$key,$value);
                    }
                }
                $sth->execute();
...
function deleteTempMailSMS(){
     $sql = "SELECT temp_email.temp_email_id FROM temp_email where TIMESTAMPDIFF(MINUTE, temp_email.created_date, now()) >= 30;
    ...
    $temp_m = array();
    foreach ($email as $m) {
        array_push($temp_m, $m['temp_email_id']);        
    }

    if (sizeof($temp_m)>0) {
        $this->app->db->beginTransaction();
        try{
            $sql2 = "INSERT INTO archive_temp_email_sms SELECT * FROM temp_email where temp_email.temp_email_id = :email";
            $sth2 = $this->app->db->prepare($sql2);
            foreach ($temp_m as $id) {
                $sth2->bindValue(':email',$id);
                $sth2->execute();
            }
            $sql3 = "DELETE from temp_email where temp_email.temp_email_id = :email";
            $sth3 = $this->app->db->prepare($sql3);
            foreach ($temp_m as $id) {
                $sth3->bindValue(':email',$id);
                $sth3->execute();
            }
            $this->app->db->commit();
        }catch(\Throwable $e){
    ...
    $temp_s = array();
      foreach ($sms as $m) {
        array_push($temp_s, $m['temp_sms_id']);        
    }
    if (sizeof($temp_s)>0) {
         $this->app->db->beginTransaction();
        try{
            $sql2 = "INSERT INTO archive_temp_email_sms SELECT * FROM temp_sms.temp_sms_id where temp_sms.temp_sms_id = :sms";
            $sth2 = $this->app->db->prepare($sql2);
            foreach ($temp_s as $id) {
                $sth2->bindValue(':sms',$id);
                $sth2->execute();
            }
            $sql3 = "DELETE from temp_sms where temp_sms.temp_sms_id = :sms";
            $sth3 = $this->app->db->prepare($sql3);
            foreach ($temp_s as $id) {
                $sth3->bindValue(':sms',$id);
                $sth3->execute();
            }
            $this->app->db->commit();
        }catch(\Throwable $e){
...
try{
    ...
    ."\nProdutos: ".$report->product->product_count
    ."\nFaturas geradas: ".$report->invoice->invoice_count
    ."\nFaturas pagas: ".$report->invoiceP->invoice_paid_count
    ."\nFaturas canceladas: ".$report->invoiceC->invoice_canceled_count
    ."\nFaturado: R$ ".$report->invoiceP->total_amount." (sub: ".$report->invoiceP->subtotal_amount." - fee: ".$report->invoiceP->fee_amount.")"
    ."\nNão Faturado: R$ ".$report->invoiceC->total_amount." (sub: ".$report->invoiceC->subtotal_amount." - fee: ".$report->invoiceC->fee_amount.")"
    ."\nNegócios: ".$report->company->company_count
    ."\nCupons: ".$report->coupon->coupon_count
    ."\nMovimentação de equipe: ".$report->employee->employee_count
    ."\nUsuários: ".$report->users->users_count
    ;
...
try{
    $sth = $this->app->db->prepare("
        SELECT count(users.user_id) as counter FROM users where TIMESTAMPDIFF(MINUTE, users.last_activity, now()) <= 30;
        SELECT count(users.user_id) as counter FROM users where TIMESTAMPDIFF(MINUTE, users.company_last_activity, now()) <= 30");
    $sth->execute();
    $viewNuppin = $sth->fetchObject();
    $sth->nextRowset();
    $viewNuppinCompany = $sth->fetchObject();
 
    $stringMessage = "Usuários ativos ultimos 30 minutos"
    ."\nConsumidor: ".$viewNuppin->counter
    ."\nParceiro: ".$viewNuppinCompany->counter;
    
    $arrayJson = array(
    "content" => $stringMessage
    );

     $this->discord_notification($arrayJson,"https://discord.com/api/webhooks/745835526732644395/rfWBF6trpL3xhgecnaFWxCJRNn7ftCE5GBllrtaPb9aNzW9eWo2WmDR8f2wc7jX9t7aj");
...
function notificationRegister($token, $userId, $source, $refreshToken){

    $this->app->db->beginTransaction();
        try{
    
        $sth2 = $this->app->db->prepare("DELETE from notification where notification_id = :token");
        $sth2->bindValue(":token",$token);
        $sth2->execute(); 

        $sql="INSERT into notification (notification_id, user_id, source, refresh_token) values(:token, :id, :source, :refresh_token);";
        $sth = $this->app->db->prepare($sql);
        ...
        $sth2 = $this->app->db->prepare("DELETE from temp_notification where temp_notification_id = :token");
        $sth2->bindValue(":token",$token);
        $sth2->execute(); 
...
function notificationTempToken($token, $source){
    $sth1 = $this->app->db->prepare("SELECT * FROM temp_notification where temp_notification.temp_notification_id = :token");
    $sth1->bindValue(":token", $token);
    $sth1->execute();
    $notification = $sth1->fetchObject();

    if(!$notification){
        $sth = $this->app->db->prepare("INSERT into temp_notification (temp_notification_id, source) values(:token, :source)");
        $sth->bindValue(":token",$token);
        $sth->bindValue(":source",$source);
        $sth->execute(); 
...
function chameSafeWay($request,$response,$args){
    return $response->withJson($this->safeWaytoPrepareOrderCompany($args["userId"]));
}
...
function getCartUser($request,$response,$args){
    ...
    if($company != false){
        ...
        $this->couponCartInfo($cartCompany, $company->info->type);
...
function addOrder($request,$response){
    ...
    try{
        ...
        $this->send_notification($safeOrder->company_id,"Pedido: ".strtoupper($orderId)." - Clique e atualize o status", "Novo Pedido", $this->app->db, "all_company");
