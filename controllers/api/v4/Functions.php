...
function getSchedulingCompany($request,$response,$args){
    ...
    date_default_timezone_set($timezone);
        ...
        $sth = $this->app->db->prepare("
            ...
            FROM scheduling
            join users on users.id = scheduling.user_id
...
function getLegal($request,$response,$args){
    $sth = $this->app->db->prepare("SELECT legal.content from legal where legal.type = :type");
...
function addTempNotification($request,$response,$args){
    ...
    $this->notificationTempToken($notification['token'], "nuppin");
}

function addTempNotificationCompany($request,$response,$args){
    $notification = $request->getParsedBody();
    $this->notificationTempToken($notification['token'], "nuppin_company");
}

function addUserNotification($request,$response,$args){
    $notification = $request->getParsedBody();
    $token = $request->getAttribute("token");
    if (!($notification['user_id'] == $token["id"])) {
        return $response->withJson(false)->withStatus(403);
    }

    try{
        $this->notificationRegister($notification['token'],$notification['user_id'], "nuppin");
    }catch(\Throwable $e){
...
function addUserNotificationCompany($request,$response,$args){
    ...
    try{
        $this->notificationRegister($notification['token'],$notification['user_id'], "nuppin_company");
    }catch(\Throwable $e){
        return $response->withJson(false)->withStatus(500);
    }
}
...
function deleteAddress($request,$response,$args){
    ...
    $sql="DELETE from address where address.id = :address and address.user_id = :userId";
    $sth = $this->app->db->prepare($sql);
    $sth ->bindValue(":userId",$address['user_id']);
    $sth ->bindValue(":address",$address['id']);
    $sth->execute();
...
function updateAddress($request,$response,$args){
    ...
    try{
        $sth1 = $this->app->db->prepare("UPDATE address SET ".implode(',', $sets)." WHERE address.id = :addressId and address.user_id = :userId");
        $sth1->bindValue(':userId',$address["user_id"]);
        $sth1->bindValue(':addressId',$address["id"]);
        foreach ($address as $key => $value) {$sth1->bindValue(':'.$key,$value);}
        $sth1->execute();

        $sth2 = $this->app->db->prepare("UPDATE address SET address.is_selected = :notSelected WHERE address.user_id = :userId and address.id != :addressId");
        $sth2->bindValue(':notSelected',"0");
        $sth2->bindValue(':addressId',$address["id"]);
...
function updateAddressToSelected($request,$response,$args){
    ...
    try{
        ...
        $sth1 =  $this->app->db->prepare( "UPDATE address SET address.is_selected = :notSelected WHERE address.user_id = :userId and address.id != :addressId");
        $sth1->bindValue(':notSelected',"0");
        $sth1->bindValue(':userId', $address['user_id']);
        $sth1->bindValue(':addressId', $address['id']);
        $sth1->execute();

        $sth2 =  $this->app->db->prepare("UPDATE address SET address.is_selected = :selected WHERE address.user_id = :userId and address.id = :addressId");
...
function addUserAddress($request,$response){
    ...
    try{
      $sql = "SELECT coupon.id FROM coupon 
            join company on company.id = coupon.company_id
            WHERE 
            (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(:longitude, :latitude), POINT(company.longitude, company.latitude))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(:longitude, :latitude),POINT(mobile.longitude, mobile.latitude))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(:longitude, :latitude), POINT(company.longitude, company.latitude))/1000)end))end) < company.max_radius and NOT EXISTS (SELECT coupon_users.coupon_id FROM coupon_users WHERE coupon_users.coupon_id = coupon.id and coupon_users.user_id = :userId);";
        ...
        $sth2 = $this->app->db->prepare("UPDATE address SET address.is_selected = :notSelected WHERE address.user_id = :userId and address.id != :lastId");
        $sth2 ->bindValue(':notSelected',"0");
        $sth2 ->bindValue(':userId', $address['user_id']);
        $sth2 ->bindValue(':lastId', $address['id']);
        $sth2 ->execute();
...
function getAddressUser($request,$response,$args){
    $token = $request->getAttribute("token");
...
function getUserSelectedAddress($request,$response,$args){
    $token = $request->getAttribute("token");
    ...
    SELECT * FROM address  where address.user_id = :userId and address.is_selected = 1 order by address.is_selected desc");
...
function getCompanyOrderByStatus($request, $response, $args){
    ...
    $sth = $this->app->db->prepare(
        ...
        item.note    
        from orders
        join order_item as item on item.order_id = orders.id
        WHERE orders.company_id = :companyId and orders.status = :position
        order by item.quantity;

        SELECT extra.* FROM orders
        JOIN order_item_extra as extra on extra.order_id = orders.id
        WHERE orders.company_id = :companyId and orders.status = :position 
        "
    );
    $sth->bindParam(":companyId", $args['companyId']);
...
function getOrderDetailUser($request,$response,$args){
    ...
    $sth = $this->app->db->prepare(
        ...
        orders.longitude
        from orders
        join company on company.id = orders.company_id
        ...
        SELECT chat.id from chat where chat.seen_date is null and chat.order_id = :orderId and chat.chat_from != :userId;
...
function getOrderUser($request,$response,$args){
    ...
    if ($args['position'] === "3") {
        ...
        from scheduling
        join company on company.id = scheduling.company_id
        where scheduling.user_id = :userId and
        company.category_company_id = :position and scheduling.source = :source
        ...
        company.category_company_id = :position and orders.source = :source
        order by orders.created_date desc";
    }
    $sth = $this->app->db->prepare($sql);
    $sth->bindParam(":userId", $args['userId']);
    $sth->bindParam(":position", $args['position']);
    $sth->bindParam(":source", $args['source']);
    $sth->bindValue(":timezone", $timezone);
    $sth->execute();
...
function updateOrderStatus($request,$response,$args){
    ...
    $sql3 = "SELECT orders.status FROM orders WHERE orders.id = :orderId";
    $sth3 = $this->app->db->prepare($sql3);
    $sth3->bindValue(':orderId', $inputOrder['id']);
    $sth3->execute();
    ...
    try{
        ...
        switch($args['idStatus']){
            case "accepted":
              $sql = "UPDATE orders SET orders.status = :status, orders.accepted_date = now() WHERE orders.id = :orderId and orders.user_id = :userId and orders.company_id = :companyId";
            break;
            case "delivery":
            case "released":
            $sql = "UPDATE orders SET orders.status = :status, orders.released_date = now() WHERE orders.id = :orderId and orders.user_id = :userId and orders.company_id = :companyId";
            break;
            ...
            case "canceled_refused":
               $sql = "UPDATE orders SET orders.status = :status, orders.completed_date = now() WHERE orders.id = :orderId and orders.user_id = :userId and orders.company_id = :companyId";
            break;
        }

        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(':status',$args['idStatus']);
        $sth->bindValue(':orderId', $inputOrder['id']);
        ...
        if ($sth->rowCount()>0) {
            if (!($inputOrder['source'] == "nuppin_company")) {
                switch ($args['idStatus']){
                    ...
                    case "canceled_refused":
                        $this->incrementProductStock($inputOrder['id']);

                        $this->send_notification_cancel_order($inputOrder['user_id'],"Pedido: ".strtoupper($inputOrder['id'])." foi cancelado pelo estabelecimento", "Pedido cancelado 😟", $this->app->db, "user");
...
function updateOrderStatusFromUser($request,$response,$inputOrder){
    $inputOrder = $request->getParsedBody();
    $token = $request->getAttribute("token");
    ...
    if ($inputOrder['status'] == "canceled_user") {
        $sql3 = "SELECT orders.status FROM orders WHERE orders.id = :orderId";
        $sth3 = $this->app->db->prepare($sql3);
        $sth3->bindValue(':orderId', $inputOrder['id']);
        $sth3->execute();
        ...
        if($order->status != "pending"){
            $error_string = "Erro ao cancelar";
            if (stristr($order->status, 'canceled')){
                $error_string = "Não é possivel cancelar, o pedido já foi cancelado";;
            }else{
                $error_string = "Não é possivel cancelar, o pedido já foi aceito pelo estabelecimento";
            }
            $error = array();
            $error['error_code'] = "001";
            $error['error_message'] = $error_string;
            $arrayJson = array("error" => $error);
            return $response->withJson($arrayJson);
        }
    }
    ...
    try{
        ...
        $sql;
        if($inputOrder['status'] == "canceled_user"){
            $sql = "UPDATE orders SET orders.status = :status, orders.completed_date = now() WHERE orders.id = :orderId and orders.user_id = :userId";
        }

        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(':status',$inputOrder['status']);
        $sth->bindValue(':orderId', $inputOrder['id']);
        $sth->bindValue(':userId', $inputOrder['user_id']);
        $sth->execute();
        ...
        if ($sth->rowCount()>0) {      
            if($inputOrder['status'] == "canceled_user"){
                ...
                $this->incrementProductStock($inputOrder['id']);
                $this->send_notification_cancel_order($inputOrder['company_id'],"Pedido: ".strtoupper($inputOrder['id'])." foi cancelado", "Pedido cancelado", $this->app->db, "all_company");
...
function getProductDetail($request,$response,$args){
    ...
    $sth= $this->app->db->prepare(
        "SELECT * FROM product WHERE product.id = :productId;

          SELECT pc.collection_id, collection.*, pc.product_id
            FROM collection
            join product_collection as pc on pc.product_id = :productId AND pc.collection_id = collection.id
            where (select count(*) from collection_extra as ce where ce.collection_id = collection.id) >= (case when collection.min_quantity = 0 then 1 end) 
            order by pc.position;

            SELECT ce.collection_id, ce.id as collection_extra_id, extra.*
            FROM collection_extra as ce
            join extra on extra.id = ce.extra_id
            WHERE extra.id = ce.extra_id and exists (select pc.collection_id from product_collection AS pc WHERE pc.product_id = :productId AND pc.collection_id = ce.collection_id);
...
function getProductAndVerifyCart($request,$response,$args){
    ...
    join cart on cart.product_id = :item AND cart.user_id = :userId and cart.id = :cartId 
    WHERE product.id = :item;
    
    SELECT pc.collection_id, c.*, pc.product_id,
    (SELECT sum(ce.quantity) FROM cart_extra as ce where ce.user_id = :userId and ce.product_id = :item and ce.collection_id = c.id and ce.cart_id = :cartId) as quantity_selected,
    (SELECT sum(ce.quantity * extra.price) FROM cart_extra as ce join extra on extra.id = ce.extra_id where ce.user_id = :userId and ce.collection_id = c.id and ce.cart_id = :cartId) as total_price
    FROM collection AS c 
    join product_collection as pc on pc.product_id = :item AND pc.collection_id = c.id
    where (select count(*) from collection_extra as ce where ce.collection_id = c.id) >= (case when c.min_quantity = 0 then 1 end)
    order by pc.position;

    SELECT ce.collection_id, ce.id as collection_extra_id, extra.*, (SELECT cart_extra.quantity FROM cart_extra where cart_extra.user_id = :userId and cart_extra.product_id = :item and cart_extra.collection_id = ce.collection_id and cart_extra.extra_id = extra.id and cart_extra.cart_id = :cartId) as quantity
    FROM collection_extra as ce
    join extra on extra.id = ce.extra_id
    ...
    SELECT size.*, (SELECT if(cart.size_id is not null,1,0) FROM cart where cart.user_id = :userId and cart.product_id = size.product_id and cart.id = :cartId and cart.size_id = size.id) as is_selected from size
    join product on product.id = :item
...
function updateQtdItemCart($request,$response,$args){
    ...
    try{
        ...
        $sql = "UPDATE cart SET cart.quantity = :qtd, cart.note = :obs, cart.size_id = :sizeId, cart.size_name = :sizeName WHERE cart.id = :cartId and cart.product_id = :item and cart.user_id = :id";
        $sth = $this->app->db->prepare($sql);
        ...
        $sth->bindValue(':cartId', $inputCart['id']);
        ...
        $sth2->bindValue(':cartId', $inputCart['id']);
        $sth2->execute();
        ...
        if (sizeof($inputCart['extra']) > 0) {
            ...
            foreach($inputCart['extra'] as $key => $value){
                ...
                foreach ($value as $key1 => $value1) {
                    if ($key1 == "cart_id") {
                            $sth3->bindValue(':'.$key1, $inputCart['id']);
                    }else if($key1 == "id"){
...
function deleteItemCart($request,$response){
    $inputCart = $request->getParsedBody();
    $token = $request->getAttribute("token");
    ...
    try{
        ...
        $sth1 = $this->app->db->prepare("DELETE from cart where cart.id = :cartId and cart.product_id = :item and cart.user_id = :id");
        $sth1->bindValue(":id",$inputCart['user_id']);
        $sth1->bindValue(":item",$inputCart['product_id']);
        $sth1->bindValue(":cartId",$inputCart['id']);
        $sth1->execute();

        if(!($inputCart["source"] == "nuppin_company")){
            $sth = $this->app->db->prepare("SELECT * FROM cart WHERE cart.user_id = :userId and cart.source = :source");
            $sth->bindValue(":userId", $inputCart["user_id"]);
            $sth->bindValue(":source", $inputCart['source']);
            $sth->execute();
            $info = $sth->fetchAll();
...
function sendCodeFromSMS($request, $response){
    ...
    $sql1 = "SELECT * from temp_sms where temp_sms.id = :phone_number";
    $sth1 = $this->app->db->prepare($sql1);
    $sth1->bindValue(':phone_number',$inputTempSms['id']);
    $sth1->execute();
    ...
    'PhoneNumber' => $inputTempSms['id'],
       ...
       try{
        ...
        $sql = "
        DELETE FROM temp_sms where temp_sms.id = :phone_number;
        INSERT INTO temp_sms(id, code_sent, attempts) VALUES(:phone_number, :code, :attempts)";
        $sth = $this->app->db->prepare($sql);
        $sth ->bindValue(':phone_number',$inputTempSms['id']);
...
function sendEmail($request, $response){
    $inputTempEmail = $request->getParsedBody();
        ...
        $sql1 = "SELECT * from temp_email where temp_email.id = :email";
        $sth1 = $this->app->db->prepare($sql1);
        $sth1->bindValue(':email',$inputTempEmail['id']);
        $sth1->execute();
    ...
    try{
        $result = $client = $this->sesEmail($inputTempEmail['id'], strtoupper($code), $this->templateEmailCode($code));
        $sql = "
        DELETE FROM temp_email where temp_email.id = :email;
        INSERT INTO temp_email(id, code_sent, attempts) VALUES(:email, :code, :attempts)";
        $sth = $this->app->db->prepare($sql);
        $sth ->bindValue(':email',$inputTempEmail['id']);
        $sth ->bindValue(':code',$code);
        $sth ->bindValue(':attempts',$attempts);
        $sth ->execute();
        return $response->withJson(true);
...
function verifyCodeFromEmail($request, $response){
    ...
    try{
        //todo new here
        $sql = "
        SELECT * FROM users WHERE users.email = :email and users.source = :source;
        SELECT * FROM temp_email WHERE temp_email.id = :email and temp_email.code_sent = :code";
        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(':code',$inputTempEmail['code_sent']);
        $sth->bindValue(':email',$inputTempEmail['id']);
        $sth->bindValue(':source',$inputTempEmail['source']);
        $sth->execute();
        $user = $sth->fetchObject();
        $sth->nextRowset();
        ...
        if($verified){
            ...
            if ($user) {
                $user->refresh_token = $this->addRefreshToken($user->id);
                return $response->withJson($user);       
            }
            ...
            $sql1 ="UPDATE temp_email set temp_email.attempts = temp_email.attempts + 1 WHERE temp_email.id = :email";
            $sth1 = $this->app->db->prepare($sql1);
            $sth1->bindValue(':email',$inputTempEmail['id']);
            $sth1->execute();
...
function verifyCodeToChangeEmail($request, $response){
    ...
    if($verified){
        if ($verified->attempts < 3) {
            $sth2 = $this->app->db->prepare("SELECT users.id FROM users WHERE users.email = :email and users.source = :source");
            $sth2->bindValue(':email',$inputTempEmail['id']);
            $sth2->bindValue(':source',$inputTempEmail['source']);
            $sth2->execute();
            $userEmail = $sth2->fetchObject();

            if (!$userEmail) {
                $sql1 = "UPDATE users set users.email = :email WHERE users.id = :userId";
                $sth1 = $this->app->db->prepare($sql1);
...
function verifyCodeFromPhoneNumber($request, $response){
    ...
    try{
        //todo new here
        $sql = "
        SELECT * FROM users WHERE users.phone_number = :phone_number and users.source = :source;
        SELECT * FROM temp_sms WHERE temp_sms.id = :phone_number and temp_sms.code_sent = :code";
        $sth = $this->app->db->prepare($sql);
        $sth ->bindValue(':code',$inputTempSms['code_sent']);
        $sth ->bindValue(':phone_number',$inputTempSms['id']);
        $sth ->bindValue(':source',$inputTempSms['source']);
        $sth->execute();
        $user = $sth->fetchObject();
        $sth->nextRowset();
        $verified = $sth->fetchObject();
        if($verified){
            ...
            $sql1 = "UPDATE temp_sms set temp_sms.attempts =  temp_sms.attempts + 1 WHERE temp_sms.id = :phone_number";
            $sth1 = $this->app->db->prepare($sql1);
            $sth1->bindValue(':phone_number',$inputTempSms['id']);
            $sth1->execute();
            return $response->withJson(2);
...
function verifyCodeToChangePhoneNumber($request, $response){
    ...
    if($verified){
        if ($verified->attempts < 3) {
             $sth2 = $this->app->db->prepare("SELECT users.id FROM users WHERE users.phone_number = :phone_number  and users.source = :source");
            $sth2->bindValue(':phone_number',$inputTempSms['id']);
            $sth2->bindValue(':source',$inputTempSms['source']);
            $sth2->execute();
            $userPhoneNumber = $sth2->fetchObject();

            if (!$userPhoneNumber) {
                $sql1 = "UPDATE users set users.phone_number = :phone_number WHERE users.id = :userId";
                $sth1 = $this->app->db->prepare($sql1);
...
function getUser($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("SELECT users.* FROM users WHERE users.id = :userId");
    $sth->bindParam("userId", $args['userId']);
    $sth->execute();
    $user = $sth->fetchObject();
    return $response->withJson($user);
...
function getUserAndAddress($request, $response, $args){
    $token = $request->getAttribute("token");
    ...
    $sth = $this->app->db->prepare("
    SELECT * FROM users WHERE users.id = :userId;
    SELECT * FROM address WHERE address.user_id = :userId and address.is_selected = :selected");
    $sth->bindParam(":userId", $args['userId']);
...
function addUser($request, $response){
    ...
    $sth1 = $this->app->db->prepare("SELECT * FROM users WHERE users.email = :email and users.source = :source");
    $sth1->bindValue(':email', $inputUser['email']);
    $sth1->bindValue(':source', $inputUser['source']);
    $sth1->execute();
    ...
    try{
        ...
        $sth->execute();
        $inputUser['id'] = $userId;
        $refreshToken = $this->addRefreshToken($inputUser["id"]);
...
function refreshAccessToken($request, $response){
    $inputToken = $request->getParsedBody();
    $token = $request->getAttribute("token");
    $jwt = $request->getAttribute("jwt");
        //todo new here

    if (!($inputToken["user_id"] == $token["id"])) {
        return $response->withJson(false)->withStatus(403);
    }

    try{
        $sth = $this->app->db->prepare("
        SELECT token.*, owner.company_id, employee.company_id as employee_company, employee.id as employee_id FROM token 
        left join employee as owner on owner.user_id = token.user_id and owner.status = 'active' and owner.is_selected = 1 and (owner.role = 'owner' or owner.role = 'admin')
        left join employee on employee.user_id = token.user_id and employee.status = 'active' and employee.role = 'employee' and employee.is_selected = 1
        WHERE token.user_id = :userId and token.id = :tokenId");

        $sth->bindValue(':userId',$token['id']);
        $sth->bindValue(':tokenId',$jwt);
        $sth->execute();
        $sthToken = $sth->fetchObject();

        if ($sthToken) {
            $accessTtoken = $this->newAccessToken($sthToken->user_id, $sthToken->company_id, $sthToken->employee_company, $sthToken->employee_id, $jwt, $inputToken["api_version"], $inputToken["aplication_version"], $inputToken["device_type"], $inputToken["source"]);        
...
function newAccessToken($userId, $companyId, $employeeCompany, $employeeId, $jwt, $apiVersion, $aplicationVersion, $deviceType, $source) {
    ...
    $sth = $this->app->db->prepare("UPDATE token SET token.refresh_date = now(), token.api_version = :apiVersion, token.aplication_version = :aplicationVersion, token.device_type = :deviceType, token.source = :source WHERE token.id = :jwt");
...
function addRefreshToken($userId) {
    ...
    try{
        ...
        $sth3 = $this->app->db->prepare("INSERT into token (id, user_id) values (:tokenId, :userId)");
...
function logoutUser($request, $response, $args){
    ...
    if (!($inputUser["id"] == $token["id"])) {
     ...
     $this->app->db->beginTransaction();
    try{
        $sth = $this->app->db->prepare("DELETE FROM token WHERE id = :token");
        $sth->bindParam(":token", $inputUser["refresh_token"]);
        $sth->execute();
...
function updateUser($request, $response){
    ...
    $inputUser["document_number"] = preg_replace("/[^0-9]/", "", $inputUser["document_number"]);

    $sthU = $this->app->db->prepare("SELECT * FROM users WHERE users.id = :userId");
    $sthU ->bindValue(':userId',$inputUser["id"]);
    $sthU->execute();
    $userU = $sthU->fetchObject();

    $sql1 = "SELECT * FROM users WHERE users.email = :email and users.source = :source;
    SELECT * FROM users WHERE users.document_number = :cpf and users.source = :source";
    $sth1 = $this->app->db->prepare($sql1);
    $sth1 ->bindValue(':email',$inputUser['email']);
    $sth1 ->bindValue(':cpf',$inputUser['document_number']);
    $sth1 ->bindValue(':source',$userU->source);
    $sth1->execute();
    ...
    if(!$user1 && !$userCpf){
        $sql = "UPDATE users SET ".implode(',', $sets)." WHERE users.id = :userId";
        $sth = $this->app->db->prepare($sql);
        $sth ->bindValue(':userId',$inputUser["id"]);
        foreach ($inputUser as $key => $value) {
            $sth ->bindValue(':'.$key,$value);
        }
        $sth->execute();
...
function updateCompanyVisibility($request, $response){
    ...
    if (!($inputCompany['id'] == $token["comp"])) {
        return $response->withJson(false)->withStatus(403);
    }

    if($inputCompany['visibility'] == 1){

            $sql1 = "SELECT company.photo, company.banner_photo FROM company WHERE company.id = :companyId";
            $sth1 = $this->app->db->prepare($sql1);
            $sth1->bindValue(':companyId', $inputCompany['id']);
            $sth1->execute();
        ...
        if($inputCompany["category_company_id"] == 3){
            $sql3 = "SELECT service.id FROM service WHERE service.company_id = :companyId";
            $sth3 = $this->app->db->prepare($sql3);
            $sth3->bindValue(':companyId', $inputCompany['id']);
            $sth3->execute();
            ...
            $sql3 = "SELECT product.id FROM product WHERE product.company_id = :companyId";
            $sth3 = $this->app->db->prepare($sql3);
            $sth3->bindValue(':companyId', $inputCompany['id']);
            $sth3->execute();
            $servico = $sth3->fetchAll();
        ...
        try{
            ...
            $sth2->bindValue(':companyId',$inputCompany['id']);
            $sth2->execute();
       
            $sth3 = $this->app->db->prepare("INSERT into validation (id, company_id) values(:validationId, :companyId)");
        ...
        $sql = "UPDATE company set company.visibility = :companyVisibility WHERE company.id = :companyId;";
        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(':companyId',$inputCompany['id']);
    ...
    $arrayJson = array();
        ...
        if($company){
           if ($company->visibility == 0) {
                ...
                $sth->bindParam(":companyId", $company->id);
                $sth->execute();
        ...
        $sthLA = $this->app->db->prepare("UPDATE company 
                ...
                where company.id = :companyId and users.id = :userId");
        $sthLA->bindParam(":companyId", $company->id);
        ...
        SELECT invoice.id from invoice where invoice.company_id = :companyId and (invoice.status != 'paid' and invoice.status != 'completed' and invoice.status != 'free');

        SELECT chat.id from chat where chat.seen_date is null and chat.chat_to = :companyId;
        ";
...
function getCompaniesByUserId($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        ...
        FROM company
        join employee on employee.company_id = company.id
        JOIN category_company as cc ON cc.id = company.category_company_id
        JOIN subcategory_company as sc ON sc.id = company.subcategory_company_id
        where employee.user_id = :userId and employee.status = 'active' and employee.is_selected = :selected and (employee.role = 'owner' or employee.role = 'admin');

        SELECT employee.* from employee
        join company on company.id = employee.company_id and employee.role = 'employee'
        ...
        truncate(plan_company.fee*100,0) as fee,
        truncate(plan_company.trial_fee*100,0) as trial_fee,
        ...
        where plan.id = plan_company.plan_id;
        ");
    $sth->bindParam(":userId", $args['userId']);
    $sth->bindValue(":selected", 1);
...
function getCompanyByEmployeeId($request, $response, $args){
    ...
    $arrayJson = array();
        ...
        $sthLA = $this->app->db->prepare("UPDATE company 
                JOIN users ON users.id = company.user_id
                SET company.last_activity = now(), users.company_last_activity = now()
                where company.id = :companyId and users.id = :userId");
        $sthLA->bindParam(":companyId", $company->id);
        $sthLA->bindValue(":userId", $args['userId']); 
...
function getCompanies($request, $response, $args){
    ...
    $sthLA = $this->app->db->prepare("UPDATE users SET users.last_activity = now() where users.id = :userId");
    $sthLA->bindValue(":userId", $args['userId']); 
    $sthLA->execute();
    ...
    $sth = $this->app->db->prepare(
        ...
        sc.name as subcategory_name,
         (case when company.model_type = 'fixed' then
            (
                IF(EXISTS(SELECT hours.company_id FROM opening_hours as hours where hours.company_id = company.id 
                and (DAYOFWEEK(:data) BETWEEN hours.weekday_id 
                and hours.weekday_end_id or (hours.weekday_id = 7 
                and hours.weekday_end_id = 1 AND (DAYOFWEEK(:data) = 7 OR DAYOFWEEK(:data) = 1))) 
                AND :hora BETWEEN if((hours.weekday_end_id > hours.weekday_id OR (hours.weekday_id = 7 
            ...
            (
            IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0)
            )
        end) AS is_online,
        (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:lo, :la))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(mobile.longitude, mobile.latitude), POINT(:lo, :la))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:lo, :la))/1000)end))end) as distance
        FROM company
        join category_company as cc on cc.id = company.category_company_id
        join subcategory_company as sc on sc.id = company.subcategory_company_id
        where
        ...
        (sum((product.price + (SELECT COALESCE(sum(size.price),0) FROM size where size.id = cart.size_id) + (SELECT COALESCE(SUM(cart_extra.quantity * extra.price),0) FROM cart_extra join extra on cart_extra.extra_id = extra.id where cart_extra.cart_id = cart.id))* cart.quantity)) total_price,
        product.company_id
        FROM cart
        ...
        cc.name as category_name,
        cc.id,
        IF(:category = cc.id,1,0) is_selected
...
function getCompaniesByCategory($request, $response, $args){
    ...
    $sth = $this->app->db->prepare(
        ...
        join category_company as cc on cc.id = company.category_company_id
        join subcategory_company as sc on sc.id = company.subcategory_company_id
        where company.visibility = 1 and
        (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:lo, :la))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(mobile.longitude, mobile.latitude), POINT(:lo, :la))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:lo, :la))/1000)end))end) < company.max_radius and company.status = :active and sc.id = :sub and (company.is_delivery = :setado or company.is_local = :setado)
        order by is_online desc, distance asc, company.rating/company.num_rating desc;
...
function addCompany($request, $response, $args){
    ...
    $sth1 = $this->app->db->prepare("SELECT * FROM company WHERE company.document_number = :document;");
    ...
    if($company){
        ...
        try{
                ...
                $this->hasIndication($userId, $companyId);
                $this->app->db->commit();        
...
function addCompanyPlan($planId, $companyId){
    $sql1 = "SELECT * FROM plan WHERE plan.id = :planId";
    $sth1 = $this->app->db->prepare($sql1);
    $sth1 ->bindValue(':planId',$planId);
    $sth1->execute();
    $plan = $sth1->fetchObject();

    $sth = $this->app->db->prepare("INSERT into plan_company (id, plan_id, company_id, price, fee, trial_period, trial_price, trial_fee, status) 
    values(:id, :planId, :companyId, :price, :fee, :trial, :trialPrice, :trialFee, :status)");
    $sth->bindValue(':id', $this->uniqidReal(9));
    $sth->bindValue(':planId',$plan->id);
...
function addFirstOwnerCompany($userId, $companyId){
    $sql1 = "SELECT users.full_name FROM users WHERE users.id = :userId";
    $sth1 = $this->app->db->prepare($sql1);
    $sth1 ->bindValue(':userId',$userId);
    $sth1->execute();
    $users = $sth1->fetchObject();

    $sth = $this->app->db->prepare("INSERT into employee (id, user_id, company_id, status, role, user_name) 
...
function deleteCompany($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("DELETE FROM company WHERE company.id = :companyId");
    $sth->bindParam("companyId", $inputCompany['id']);
    $company =$sth->execute();
    return $response->withJson($company);
}
...
function updateCompany($request,$response,$args){
    ...
    $sql = "UPDATE company SET ".implode(',', $sets)." WHERE company.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth ->bindValue(':id',$inputCompany['id']);
    foreach ($inputCompany as $key => $value) {
        if($key != "is_selected"){// todo - retirar após correção do bug na versão 1.2.0
            $sth ->bindValue(':'.$key,$value);
        }
...
function OrderRating($request, $response){
    ...
    FROM company 
    join employee on employee.company_id = company.id
    where employee.user_id = :userId and employee.status = 'active' and company.id = :companyId";
    $sth1 = $this->app->db->prepare($sql1);
    ...
    WHERE company.id = :companyId AND orders.id = :orderId and orders.rating < 1;
    ";
    $sth = $this->app->db->prepare($sql);
...
function SchedulingRating($request, $response){
    ...
    company.rating = company.rating+:rating, 
    company.num_rating = company.num_rating+:plusOne
    WHERE company.id = :companyId AND scheduling.id = :schedulingId and scheduling.rating < 1;
    ";
    $sth = $this->app->db->prepare($sql);
    ...
    $sth->bindValue(':schedulingId',$inputScheduling['id']);
    $sth->execute();
        if ($sth->rowCount() > 0) {
...
function getCompanyDescription($request, $response, $args){
    //TODO TOKEN HERE
     ...
     $sth = $this->app->db->prepare("
        SELECT * from company where company.id = :companyId;
        ...
        SELECT * from payment where EXISTS (select payco.company_id from payment_company as payco where payco.company_id = :companyId and payment.id = payco.payment_id);
        ");     
...
function getCompanyDomain($request, $response, $args){
    
    $sth = $this->app->db->prepare("SELECT * from company where company.site = :domain;");     
    $sth->bindValue(':domain',$args['domain']);
    $sth->execute();
    $company = $sth->fetchObject();
...
function getCompanyApp($request, $response, $args){
    
    $sth = $this->app->db->prepare("
    SELECT * from company where company.id = :companyId;
    SELECT * from address where address.user_id = :userId and address.is_selected = 1
    ");     
    $sth->bindValue(':companyId',$args['companyId']);
    $sth->bindValue(':userId',$args['userId']);
    $sth->execute();
...
function getProduct($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        ...
        if((select count(*) from collection_extra as ce where ce.collection_id = collection.id) >= (case when collection.min_quantity = 0 then 1 end),0,1) as has_warning
        FROM collection
        join product_collection as pc where pc.product_id = :productId AND pc.collection_id = collection.id
        order by pc.position;

        SELECT ce.collection_id, ce.id, extra.* FROM collection_extra AS ce 
        join extra on extra.id = ce.extra_id
        WHERE extra.id = ce.extra_id and exists (select pc.collection_id from product_collection AS pc WHERE pc.product_id = :productId AND pc.collection_id = ce.collection_id);
...
function getListProductAndVerifyCart($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        ...
        FROM cart
        join product on cart.product_id = product.id
        where cart.user_id = :userId and cart.source = :source");

    $sth->bindParam(":companyId", $args['companyId']);
    $sth->bindParam(":userId", $args['userId']);
    $sth->bindValue(":source", $args['source']);
    $sth->execute();
...
function addProduct($request, $response){
    ...
    if($inputProduct["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT product.id FROM product WHERE product.external_code = :externalId and product.company_id = :companyId");
...
function updateProduct($request,$response,$args){
    ...
    if($inputProduct["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT product.id FROM product WHERE product.external_code = :externalId and product.company_id = :companyId and product.id != :productId");
        $sth2->bindParam("externalId", $inputProduct['external_code']);
        $sth2->bindParam("companyId", $inputProduct['company_id']);
        $sth2->bindParam("productId", $inputProduct['id']);
        $sth2->execute();
        $check = $sth2->fetchObject();
    ...
    $sql = "UPDATE product SET ".implode(',', $sets)." WHERE product.id = :id";
    $sth = $this->app->db->prepare($sql);
...
function deleteProduct($request,$response,$args){
    ...
    try{
        ...
        $sth = $this->app->db->prepare("DELETE FROM product WHERE product.id = :id and product.company_id = :companyId");
        $sth->bindParam(":id", $inputProduct['id']);
        ...
        if ($affectedRows > 0) {
            ...
            $this->deleteS3("product",$inputProduct["id"]);
            }
...
function getCoupon($request, $response, $args){
    ...
    left join coupon_users as uc on uc.coupon_id = coupon.id 
    WHERE coupon.company_id = :companyId
    and TIMESTAMPDIFF(MINUTE, now(), coupon.due_date) >= 1 group by coupon.id order by coupon.created_date";
...
function getCouponUserFromCart($request, $response, $args){
    ...
    join coupon on coupon.id = uc.coupon_id and coupon.company_id = :companyId
    join company on company.id = coupon.company_id
...
function getCouponUsers($request, $response, $args){
    ...
    FROM coupon_users as uc
    join coupon on coupon.id = uc.coupon_id
    join company on company.id = coupon.company_id
    ...
    foreach ($coupon as $i => $item) {
        ...
        $sql1 = "SELECT * from company where company.id = :companyId";
        $sth1 = $this->app->db->prepare($sql1);
...
function updateStatusScheduling($request, $response, $args){
    ...
    $sql3 = "SELECT scheduling.status FROM scheduling WHERE scheduling.id = :schedulingId";
    $sth3 = $this->app->db->prepare($sql3);
    $sth3->bindValue(':schedulingId', $inputScheduling['id']);
    $sth3->execute();
    ...
    try{
        ...
        switch($args['idStatus']){
            case "accepted":
            $sql = "UPDATE scheduling SET scheduling.status = :status, scheduling.accepted_date = now() WHERE scheduling.id = :schedulingId and scheduling.user_id = :userId and scheduling.company_id = :companyId";
            break;
            case "concluded_not_rated":
            ...
            case "canceled_refused":
            case "concluded":
               $sql = "UPDATE scheduling SET scheduling.status = :status, scheduling.completed_date = now() WHERE scheduling.id = :schedulingId and scheduling.user_id = :userId and  scheduling.company_id = :companyId";
            break;
        }
        $sth = $this->app->db->prepare($sql);
        $sth ->bindValue(':status',$args['idStatus']);
...
function updateStatusSchedulingFromUser($request, $response, $args){
    ...
    $sql3 = "SELECT * FROM scheduling WHERE scheduling.id = :schedulingId";
    $sth3 = $this->app->db->prepare($sql3);
    $sth3->bindValue(':schedulingId', $inputScheduling['id']);
    $sth3->execute();
    $scheduling = $sth3->fetchObject();
    ...
    switch($args['idStatus']){
        case "canceled_user":
           $sql = "UPDATE scheduling SET scheduling.status = :status, scheduling.completed_date = now() WHERE scheduling.id = :schedulingId and scheduling.user_id = :userId";
        break;
    }
    $sth = $this->app->db->prepare($sql);
    $sth ->bindValue(':status',$args['idStatus']);
    ...
    if ($sth->rowCount() > 0) {
        ...
        if(!($scheduling->source == "nuppin_company")){
            if ($args['idStatus'] == "canceled_user"){
                $this->send_notification_cancel_scheduling($inputScheduling['company_id'],$inputScheduling['employee_id'],"Agendamento: ".strtoupper($inputScheduling['id'])." foi cancelado", "Agendamento Cancelado", $this->app->db, "company_scheduling");
...
function getScheduling($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        ...
        SELECT chat.id from chat where chat.seen_date is null and chat.order_id = :schedulingId and chat.chat_from != :userId;");
...
function getEmployeeScheduling($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        SELECT employee.id, employee.user_name, employee.start_time, employee.end_time, service.duration, hours.end_time as company_end_time, hours.start_time as company_start_time FROM service_employee AS servemp 
        join employee on employee.id = servemp.employee_id and employee.status = :status
        join service on service.id = servemp.service_id
        join opening_hours as hours on hours.company_id = service.company_id
        WHERE servemp.service_id = :serviceId and employee.id = servemp.employee_id and hours.weekday_id = DAYOFWEEK(:data);
                ...
                if($i >= strtotime($hour) || $dayUser > $day){
                    ...
                    if(strtotime("+$event_length minutes", $i) <= strtotime($time_block)){
                        ...
                        $novo = array(
                            ...
                            "employee_id"=>$employee[$k]['id'],
                            "employee_name"=>$employee[$k]['user_name']);
                        $todos[$i] = $novo;
                    }

                    for($j = 0; $j < sizeof($scheduling);$j++){
                        if ((strtotime($scheduling[$j]['end_time']) >= strtotime("+$event_length minutes", $i) && strtotime($scheduling[$j]['start_time']) < strtotime("+$event_length minutes", $i) && $employee[$k]['id'] == $scheduling[$j]['employee_id'])
                            ||
                            (strtotime($scheduling[$j]['end_time']) <= strtotime("+$event_length minutes", $i) && strtotime($scheduling[$j]['end_time']) > $i && $employee[$k]['id'] == $scheduling[$j]['employee_id'])
                        ){
                            $new_event_time = date('H:i', $i);
                        $termino = strtotime("+$event_length minutes", $i);
                        $new_event_time2 = date('H:i', $termino);
...
function addScheduling($request, $response){
    ...
    try{
        ...
        if ($sth->execute()) {
            ...
            if (!($inputScheduling["source"] == "nuppin_company")) {
                $this->send_notification_new_scheduling($inputScheduling["company_id"],$inputScheduling["employee_id"],"Clique e atualize o status", "Você tem um novo agendamento", $this->app->db,"company_scheduling");
        ...
        $this->app->db->commit();
        $inputScheduling['id'] = $schedulingId;
...
function updateService($request,$response,$args){
    ...
    $sql = "UPDATE service SET ".implode(',', $sets)." WHERE service.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth ->bindValue(':id',$inputService['id']);
    foreach ($inputService as $key => $value) {
        $sth ->bindValue(':'.$key,$value);
    }
    $sth->execute();
...
function deleteService($request,$response,$args){
    ...
    try{
        ...
        $sth1->bindParam(":id", $inputService['id']);
        $sth1->execute();

        $sth2 = $this->app->db->prepare("DELETE FROM service WHERE service.id = :id");
        $sth2->bindParam(":id", $inputService['id']);
        $sth2->execute();
...
function addEmployee($request, $response){
    ...
    $sth2 = $this->app->db->prepare("
        ...
        not EXISTS(SELECT employee.user_id from employee where (employee.user_id = users.id and employee.status = :status and employee.company_id = :companyId) or (employee.user_id = users.id and employee.status = :status and employee.company_id != :companyId and employee.role = :role))");
    ...
    if($user){
        ...
        $inputEmployee["user_id"] = $user->id;//here we set the id to employee.user_id
...
function getListEmployeeService($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        ...
        FROM employee
        join service_employee as servemp on servemp.service_id = :serviceId and employee.id = servemp.employee_id
...
function getListEmployeeNotService($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        ...
        DATE_FORMAT(employee.end_time, '%H:%i') as end_time, 
        employee.title 
        FROM employee WHERE employee.company_id = :companyId and employee.status = :status AND NOT EXISTS (SELECT employee.id FROM service_employee as servemp WHERE servemp.service_id = :id and employee.id = servemp.employee_id)");
...
function updateEmployee($request,$response){
    ...
    $sql = "UPDATE employee SET ".implode(',', $sets)." WHERE employee.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth ->bindValue(':id',$inputEmployee['id']);
    foreach ($inputEmployee as $key => $value) {
        $sth ->bindValue(':'.$key,$value);
    }
    $sth->execute();
...
function deleteEmployee($request,$response,$args){
    ...
    if (!($inputEmployee['company_id'] == $token["comp"] || $inputEmployee['id'] == $token["emp"])) {
        return $response->withJson(false)->withStatus(403);
    }
    ...
    try{
        ...
        $sth1->bindParam(":employeeId", $inputEmployee['id']);
        $sth1->execute();

        $sth2 = $this->app->db->prepare("UPDATE employee SET employee.status = :status, employee.fired_date = now() WHERE employee.id = :employeeId and employee.role = :role");
        $sth2->bindParam(":employeeId", $inputEmployee['id']);
...
function deleteEmployeeService($request,$response){
    $inputServiceEmployee = $request->getParsedBody();
    $token = $request->getAttribute("token");
    
    if (!($inputServiceEmployee['company_id'] == $token["comp"])) {
        return $response->withJson(false)->withStatus(403);
    }
    ...
    $sth->bindParam(":employeeId", $inputServiceEmployee['employee_id']);
    $sth->bindParam(":servId", $inputServiceEmployee['service_id']);
    $sth->execute();
    $affectedRows = $sth->rowCount();
    return $response->withJson($affectedRows);
}
...
function getCompanyReport($request, $response, $args){
    ...
    FROM company 
    left join orders on orders.company_id = company.id and date(CONVERT_TZ(orders.created_date,'UTC',:timezone)) BETWEEN :data1 and :data2
    WHERE company.id = :companyId;

    SELECT count(*) AS recurring_customer
    FROM (
    SELECT o.user_id FROM orders as o where o.company_id = :companyId and o.status LIKE :concluded and date(CONVERT_TZ(o.created_date,'UTC',:timezone)) BETWEEN :data1 AND :data2 and exists(select o2.user_id from orders as o2 where o2.id != o.id and o2.user_id = o.user_id AND o2.company_id = o.company_id)
    GROUP BY o.user_id)AS t;

    SELECT count(*) AS new_customer
    FROM (
    SELECT o.user_id FROM orders as o where o.company_id = :companyId and o.status LIKE :concluded and date(CONVERT_TZ(o.created_date,'UTC',:timezone)) BETWEEN :data1 AND :data2 and not exists(select o2.user_id from orders as o2 where o2.id != o.id and o2.user_id = o.user_id AND o2.company_id = o.company_id)
    ...
    FROM company
    left join view_count on view_count.company_id = company.id and date(view_count.created_date) BETWEEN :data1 and :data2
    WHERE company.id = :companyId;
    ");
...
function getCompanyReportScheduling($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        ...
        FROM company
        left join scheduling on scheduling.company_id = company.id and date(scheduling.start_time) BETWEEN :data1 and :data2
        WHERE company.id = :companyId;

        SELECT count(*) AS recurring_customer
        FROM (
        SELECT o.user_id FROM scheduling as o where o.company_id = :companyId and o.status LIKE :concluded and date(o.start_time) BETWEEN :data1 AND :data2 and exists(select o2.user_id from scheduling as o2 where o2.id != o.id and o2.user_id = o.user_id AND o2.company_id = o.company_id)
        GROUP BY o.user_id)AS t;

        SELECT count(*) AS new_customer
        FROM (
        SELECT o.user_id FROM scheduling as o where o.company_id = :companyId and o.status LIKE :concluded and date(o.start_time) BETWEEN :data1 AND :data2 and not exists(select o2.user_id from scheduling as o2 where o2.id != o.id and o2.user_id = o.user_id AND o2.company_id = o.company_id)
        GROUP BY o.user_id)AS t2;
...
function getOrderRating($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("SELECT upper(orders.id) as id, orders.rating, date(orders.created_date) as created_date, orders.rating_note FROM orders WHERE orders.company_id = :companyId and date(CONVERT_TZ(orders.created_date,'UTC',:timezone)) BETWEEN :data1 and :data2 and orders.rating > 0 order by orders.created_date desc;");
    $sth->bindValue(":companyId", $args['companyId']);  
    $sth->bindValue(":data1", $args['data1']);
    $sth->bindValue(":data2", $args['data2']);
    $sth->bindValue(":timezone", $timezone);
...
function deleteCompanyPayment($request,$response){
    ...
    $sth1 = $this->app->db->prepare("
        SELECT * from payment where EXISTS (select payco.company_id from payment_company as payco where payco.company_id = :companyId and payment.id = payco.payment_id)
        ");     
    $sth1->bindValue(":companyId", $inputPaycom['company_id']);
    $sth1->execute();
...
function getPaymentMethod($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        SELECT payment.id as payment_id , payment.name, 
        IF(EXISTS (select * from payment_company as payco where payco.company_id = :companyId and payment.id = payco.payment_id),1,0)as is_checked 
        from payment;
        ");     
    $sth ->bindValue(':companyId',$args['companyId']);
    $sth->execute();
...
function getCompanySchedule($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        ...
        from opening_hours as hours where hours.company_id = :companyId order by hours.weekday_id;
        ");     
    $sth ->bindValue(':companyId',$args['companyId']);
    $sth->execute();
    $dias = $sth->fetchAll();
...
function getUndefinedCompanySchedule($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        SELECT weekday.id as weekday_id from weekday where not exists(select hours.company_id from opening_hours as hours where hours.company_id = :companyId AND hours.weekday_id = weekday.id)
        ");     
    $sth ->bindValue(':companyId',$args['companyId']);
    $sth->execute();
    $dias = $sth->fetchAll();
...
function mobileCompanyUpdate($request, $response){
    ...
    $sth2 = $this->app->db->prepare("SELECT mobile.id from mobile where mobile.company_id = :companyId and mobile.latitude = 0 and mobile.longitude = 0");
    $sth2->bindValue(':companyId',$inputMobile['company_id']);
    $sth2->execute();
    $mobile2 = $sth2->fetchObject();

    if($mobile2){
        $sql3 = "UPDATE mobile SET mobile.latitude = :latitude, mobile.longitude = :longitude WHERE mobile.id = :mobileId";
        $sth3 = $this->app->db->prepare($sql3);
        $sth3 ->bindValue(':mobileId',$mobile2->id);
...
function getInvoice($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("SELECT * from invoice where invoice.company_id = :companyId order by invoice.created_date desc;");     
    $sth ->bindValue(':companyId',$args['companyId']);
    $sth->execute();
    $invoice = $sth->fetchAll();
...
function getInvoiceDetail($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        ...
        left join invoice_payment as ip on ip.invoice_id = invoice.id
        join company on company.id = invoice.company_id
        where invoice.id = :id;
        ");     
...
function getInvoiceOrderDetail($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("
        ...
        LEFT JOIN invoice ON invoice.id = (SELECT  i.id FROM invoice as i WHERE i.company_id = :id and i.created_date < :createdDate order by i.created_date desc limit 1)
...
function getPlan($request, $response, $args){
    ...
    $sql = "SELECT plan.id, plan.name, plan.icon, plan.nivel, plan.price, truncate(plan.fee*100,0) as fee, plan.trial_period, plan.trial_price, truncate(plan.trial_fee*100,0) as trial_fee from plan where plan.category_company_id >= '88';
    SELECT pb.plan_id, benefit.description FROM plan
    join plan_benefit AS pb on pb.plan_id = plan.id
    join benefit on benefit.id = pb.benefit_id
    where plan.category_company_id >= '88' order by benefit.position;
    ";

    if($args['categoryId'] == "3"){
        $sql = "SELECT plan.id, plan.name, plan.icon, plan.nivel, plan.price, truncate(plan.fee*100,0) as fee, plan.trial_period, plan.trial_price, truncate(plan.trial_fee*100,0) as trial_fee from plan where category_company_id = '99';
        SELECT pb.plan_id, benefit.description FROM plan
        join plan_benefit AS pb on pb.plan_id = plan.id
        join benefit on benefit.id = pb.benefit_id
        where plan.category_company_id = '99' order by benefit.position;
        ";   
    }
    ...
    foreach ($plan as &$p) {
        ...
        foreach ($benefit as $b) {
            if ($p['id'] === $b['plan_id']) {
                $benefitObj = new \stdClass();
                $benefitObj->description = $b['description'];
                array_push($p['benefit'],$benefitObj);
            }
        }
    }
...
function updateFinance($request,$response,$args){
    ...
    $sql = "UPDATE finance SET ".implode(',', $sets)." WHERE finance.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth->bindValue(':id',$inputFinance['id']);
    foreach ($inputFinance as $key => $value) {
        $sth->bindValue(':'.$key,$value);
    }
    $sth->execute();
...
function deleteFinance($request, $response, $args){
    ...
    $sth = $this->app->db->prepare("DELETE FROM finance WHERE finance.id = :financeId and finance.company_id = :companyId");
    $sth->bindParam(":companyId", $inputFinance['company_id']);
    $sth->bindParam(":cashflowId", $inputFinance['id']);
    $sth->execute();

     if ($input["photo"]) {
        $this->deleteS3("finance",$inputFinance["id"]);
    }
...
function updateCanceledBoleto($request, $response, $args){
    ...
    if($data){
        $this->app->db->beginTransaction();
        try{
            $this->generateInvoiceBoleto($documentType, $inputInvoice['price'], $inputInvoice['fee'], $data->email, $data->full_name, $data->document_number, $data->name, $inputInvoice['id'], $data->companyId);
            $this->app->db->commit();
...
function getPosts($request, $response, $args){
    $sth = $this->app->db->prepare("
        ...
        FROM material
        join category_material as mc on mc.id = material.category_material_id
        join subcategory_material as ms on ms.id = material.subcategory_material_id
        where mc.id = material.category_material_id and ms.id = material.subcategory_material_id and material.status = 1 order by material.created_date desc LIMIT 10 OFFSET :inicio");
...
function getMaterialBody($request, $response, $args){
    $sth = $this->app->db->prepare(
        ...
        WHERE material.id = :materialId and material.status = 1; 
        UPDATE material SET material.view_counter = material.view_counter + 1 where material.id = :materialId"
    );
...
//FUNÇÃO PARA DELETAR IMAGENS NO AWS S3
        ...
        if($result['ObjectURL'] != null){
            ...
            switch($args['folder']){
                case "company":
                    ...
                    $sql = "UPDATE company SET company.photo = :key where company.id = :id";
                break;
                case "company_banner":
                    $this->instagrammer("Empresa adicionou foto de banner 🖼\nLink: ".$result['ObjectURL']);
                    $sql = "UPDATE company SET company.banner_photo = :key where company.id = :id";
                break;
                case "product":
                    $this->instagrammer("Foto adicionada ao produto 🖼\nLink: ".$result['ObjectURL']);
                    $sql = "UPDATE product SET product.photo = :key where product.id = :id";
                break;
                case "finance":
                    $this->mrGoodNews("Foto adicionada ao registro financeiro 🖼");
                    $sql = "UPDATE finance SET finance.photo = :key where finance.id = :id";
                break;
                case "users":
                    $this->mrGoodNews("Usuário adicionou foto de perfil 🖼");
                    $sql = "UPDATE users SET users.photo = :key where users.id = :id";
                break;
            }
    ...
    try{
        ...
        if($status->status_request->transaction_id != null){
            ...
            if ($boleto['status'] == 'paid') {
                $sthcheck = $this->app->db->prepare("SELECT ip.status from invoice_payment as ip where ip.id = :transactionId ");
                $sthcheck->bindValue(':transactionId',$boleto['transaction_id']);
                $sthcheck->execute();
                $ip = $sthcheck->fetchObject();

                if($ip->status == "paid" || $ip->status == "completed"){// assegura que a comissão não vai ser pago 2x caso paghiper erre e chame paid 2x
                    return $response->withStatus(200);
                }

                $boleto['value_cents'] = $status->status_request->value_cents;
                $boleto['value_fee_cents'] = $status->status_request->value_fee_cents;

                $sth3 = $this->app->db->prepare("SELECT company.id, company.status from invoice
                join invoice_payment as ip on ip.invoice_id = invoice.id
                join company on company.id = invoice.company_id
                where ip.id = :transactionId AND company.status != :active");
            ...
            if($company){
                $sql2 = "
                UPDATE company
                INNER JOIN invoice ON (invoice.company_id = company.id)
                INNER JOIN invoice_payment as ip on ip.invoice_id = invoice.id
                SET company.status = :active,
                invoice.status = :status,
                invoice.completed_date = :hoje,
                ip.status = :status,
                ip.completed_date = :hoje
                where company.id = :companyId and ip.id = :transactionId and invoice.id = ip.invoice_id";
                $sth2 = $this->app->db->prepare($sql2);
                $sth2->bindValue(":active",'active');
                $sth2->bindValue(":hoje",date("Y-m-d"));
                $sth2->bindValue(":companyId",$company->id);
                $sth2->bindValue(':transactionId',$boleto['transaction_id']);
                $sth2->bindValue(':status',$boleto['status']);
                $sth2->execute();
                $this->mrGoodNews("Boleto pago 🤑");
            }else{
                $sql2="
                UPDATE invoice
                INNER JOIN invoice_payment as ip on ip.invoice_id = invoice.id
                ...
                ip.status = :status,
                ip.completed_date = :hoje
                where ip.id = :transactionId and invoice.id = ip.invoice_id";
                ...
                $sth4 = $this->app->db->prepare("SELECT company.id, invoice.id as invoice_id from invoice
                join invoice_payment as ip on ip.invoice_id = invoice.id
                join company on company.id = invoice.company_id
                where ip.id = :transactionId");
                $sth4->bindValue(':transactionId',$boleto['transaction_id']);
                $sth4->execute();
                $company = $sth4->fetchObject();

                $sth5 = $this->app->db->prepare("DELETE from invoice_payment as ip where ip.invoice_id = :invoiceId and ip.id != :transactionId");
                $sth5->bindValue(':invoiceId',$company->invoice_id);
                $sth5->bindValue(':transactionId',$boleto['transaction_id']);
                $sth5->execute();

                $this->cashInAffiliate($company->id, $boleto['value_cents'], $boleto['value_fee_cents']);

                $this->send_notification_invoice($company->id, "Que bom que deu tudo certo", "Sua fatura está paga! 😍", $this->app->db, "company");
...
function companyOrUser($tag){
    ...
    switch ($tag){
        case "user":
            return "SELECT notification.id as token FROM notification 
            WHERE notification.user_id = :id and notification.source != 'nuppin_company'";
        case "company":
            return "SELECT notification.id as token FROM notification
        ...
        case "company_scheduling":
            return "SELECT notification.id as token FROM notification 
            join employee on employee.user_id = notification.user_id AND notification.source = 'nuppin_company' AND employee.status = 'active' AND (employee.role = 'owner' or employee.role = 'admin' or employee.id = :employeeId)
            WHERE employee.company_id = :companyId";
        case "coupon":
            return "SELECT notification.id FROM coupon_users AS uc
            JOIN company ON company.id = :companyId
            JOIN notification ON notification.user_id = uc.user_id and notification.source != 'nuppin_company'
...
function reminderInvoicePayment(){
    $this->app->db->beginTransaction();
    try{
        $sql = "SELECT invoice.id, invoice.company_id FROM invoice
        ...
        if(sizeof($data)>0){
            foreach ($data as $index) {
                ...
                $sql3 = "UPDATE invoice set invoice.reminder_count = 1 where invoice.id = :invoiceId";
                $sth3 = $this->app->db->prepare($sql3);
                $sth3->bindValue(':invoiceId',$index['id']);
                $sth3->execute();           
            }
        }
        $this->app->db->commit();
    ...
    try{
        ...
        if(sizeof($data)>0){
            foreach ($data as $index) {
                ...
                $sql3 = "UPDATE invoice set invoice.reminder_count = 2 where invoice.id = :invoiceId";
                $sth3 = $this->app->db->prepare($sql3);
                $sth3->bindValue(':invoiceId',$index['id']);
                $sth3->execute();
            }
...
function deleteTempMailSMS(){
    $sth = $this->app->db->prepare("
        SELECT temp_email.id FROM temp_email where TIMESTAMPDIFF(MINUTE, temp_email.created_date, now()) >= 30;
        SELECT temp_sms.id FROM temp_sms where TIMESTAMPDIFF(MINUTE, temp_sms.created_date, now()) >= 30");
    $sth->execute();
    $email = $sth->fetchAll();
    $sth->nextRowset();
    $sms = $sth->fetchAll();
    ...
    if (sizeof($email)>0) {
        $this->app->db->beginTransaction();
        try{
            $sth2 = $this->app->db->prepare("INSERT INTO archive_temp_email_sms SELECT * FROM temp_email where temp_email.id = :email");
            foreach ($email as $e) {

                $sth2->bindValue(':email',$e["id"]);
                $sth2->execute();

                $sth3 = $this->app->db->prepare("DELETE from temp_email where temp_email.id = :email");
                $sth3->bindValue(':email',$e["id"]);
                $sth3->execute();
            }
    ...
    if (sizeof($sms)>0) {
         $this->app->db->beginTransaction();
        try{
            $sth2 = $this->app->db->prepare("INSERT INTO archive_temp_email_sms SELECT * FROM temp_sms where temp_sms.id = :sms");
            foreach ($sms as $s) {
                $sth2->bindValue(':sms',$s["id"]);
                $sth2->execute();

                $sth3 = $this->app->db->prepare("DELETE from temp_sms where temp_sms.id = :sms");
                $sth3->bindValue(':sms',$s["id"]);
                $sth3->execute();
            }
            $this->app->db->commit();
        }catch(\Throwable $e){
...
function putOffCompanyMobile(){
    ...
    join company on company.id = mobile.company_id
...
function deleteChat(){
     ...
     join orders on orders.id = chat.order_id
     ...
     join scheduling on scheduling.id = chat.order_id
...
function suspendCompany(){
        $sql = "SELECT invoice.company_id from invoice
                where invoice.due_date < :hoje and invoice.status != :paid and invoice.status != :completed and invoice.status != :free and not exists(select company.status from company  where company.id = invoice.company_id and company.status = :suspended)";
...
function generateInvoice($request, $response){
    ...
    try{
        ...
        FROM company
        LEFT JOIN invoice ON invoice.id = (SELECT invoice2.id FROM invoice as invoice2 WHERE invoice2.company_id = company.id order by invoice2.created_date desc limit 1)
        join employee on employee.company_id = company.id and employee.role = 'owner' and employee.status = 'active'
        join users on users.id = employee.user_id
        join plan_company as pc on pc.company_id = company.id
        ...
        if(sizeof($data) != 0){
                $sth2 = $this->app->db->prepare("INSERT into invoice(id, company_id, due_date, created_date, subtotal_amount, fee_amount, total_amount, status, completed_date) 
...
function generateInvoiceFree($sth2, $createdDate, $companyStatus, $companyId){
    ...
    if ($companyStatus != "active") {
        $sql="UPDATE company set company.status = :status where company.id = :companyId";
        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(':status',"active");
        $sth->bindValue(':companyId',$companyId);
        $sth->execute();
...
function generateInvoiceBoleto($documentType, $price, $fee, $email, $fullName, $documentNumber, $companyName, $invoiceId, $companyId){
    ...
    try{
        $sql2 ="INSERT into invoice_payment(id, invoice_id, status, amount, created_date, due_date, code_line, external_link, completed_date) 
        values (:id, :invoice_id, :status, :amount, :created_date, :due_date, :code_line, :external_link, :completed_date)";
        $sth2 = $this->app->db->prepare($sql2);
        $sth2->bindValue(':id',$boleto->create_request->transaction_id);
        $sth2->bindValue(':invoice_id',$invoiceId);
        ...
        if ($boleto->create_request->transaction_id != null) {
            $sth2->execute();
            $sql="UPDATE invoice set invoice.status = :status where invoice.id = :invoiceId";
            $sth = $this->app->db->prepare($sql);
            ...
            $sth = $this->app->db->prepare("DELETE from invoice where invoice.id = :invoiceId");
            $sth->bindValue(':invoiceId',$invoiceId);
            $sth->execute();
        }
    }catch(\Throwable $e){
...
function geraComissao($companyId, $category, $invoiceCreatedDate, $invoiceLastCreatedDate, $fee){
    ...
    if($inputCollection["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT collection.id FROM collection WHERE collection.external_code = :externalId and collection.company_id = :companyId");
    ...
    $sth->execute();
    $inputCollection['id'] = $collectionId;
...
function updateCollection($request,$response,$args){
    ...
    if($inputCollection["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT collection.id FROM collection WHERE collection.external_code = :externalId and collection.company_id = :companyId and collection.id != :collectionId");
        $sth2->bindParam("externalId", $inputCollection['external_code']);
        $sth2->bindParam("companyId", $inputCollection['company_id']);
        $sth2->bindParam("collectionId", $inputCollection['id']);
        $sth2->execute();
        $check = $sth2->fetchObject();
    ...
    $sql = "UPDATE collection SET ".implode(',', $sets)." WHERE collection.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth->bindValue(':id',$inputCollection['id']);
    foreach ($inputCollection as $key => $value) {
        $sth->bindValue(':'.$key,$value);
    }
    $sth->execute();
...
function deleteCollection($request,$response,$args){
    ...
    try{
        ...
        $sth2 = $this->app->db->prepare("DELETE FROM collection WHERE collection.id = :collectionId");
        $sth2->bindParam(":collectionId", $inputCollection['id']);
        $sth2->execute();
...
function getListCollection($request,$response,$args){
    ...
    SELECT collection.*,
    if((select count(*) from collection_extra as ce where ce.collection_id = collection.id) >= (case when collection.min_quantity = 0 then 1 end),0,1) as has_warning,
    if((select count(*) from collection_extra as ce2 where ce2.collection_id = collection.id) = 0,1,0) as is_empty
...
function getListCollectionNotProduct($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        SELECT collection.name, collection.id as collection_id, collection.company_id FROM collection WHERE collection.company_id = :companyId and
        not exists (select * from product_collection as pc where pc.collection_id = collection.id and product_id = :productId) and exists (select * from collection_extra as ce where ce.collection_id = collection.id) and collection.min_quantity <= (select count(*) from collection_extra as ce2 where ce2.collection_id = collection.id)
...
function getListExtraNotCollection($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
    SELECT extra.id as extra_id, extra.name, extra.company_id FROM extra WHERE extra.company_id = :companyId and
    not exists (select * from collection_extra as ce where ce.collection_id = :collectionId and ce.extra_id = extra.id)");
...
function getCollection($request,$response,$args){
    ...
    $sth = $this->app->db->prepare("
        SELECT collection.*,
        if((select count(*) from collection_extra as ce where ce.collection_id = collection.id)>= (case when collection.min_quantity = 0 then 1 end),0,1) as has_warning
        FROM collection WHERE collection.id = :collectionId;
        
        SELECT extra.*, ce.collection_id from extra
        join collection_extra as ce on ce.collection_id = :collectionId and extra.id = ce.extra_id;");
...
function addCollectionExtra($request, $response){
    ...
    foreach ($inputCollection as $key => $value) {
        ...
        $sql = "INSERT INTO collection_extra (id,".implode(',', $keys).") VALUES (:id, :".implode(",:", $keys).")";
        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(":id", $this->uniqidReal(9));
        foreach ($value as $key1 => $value1) {
            $sth->bindValue(':'.$key1,$value1);
...
function deleteCollectionExtra($request,$response){
    ...
    $sth->bindParam(":collectionId", $inputCollection['id']);
...
function deleteCollectionProduct($request,$response){
  ...
  function addExtra($request, $response){
    ...
    if($inputExtra["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT extra.id FROM extra WHERE extra.external_code = :externalId and extra.company_id = :companyId");
...
function updateExtra($request,$response,$args){
    ...
    if($inputExtra["external_code"] != ""){
        $sth2 = $this->app->db->prepare("SELECT extra.id FROM extra WHERE extra.external_code = :externalId and extra.company_id = :companyId and extra.id != :extraId");
        $sth2->bindParam("externalId", $inputExtra['external_code']);
        $sth2->bindParam("companyId", $inputExtra['company_id']);
        $sth2->bindParam("extraId", $inputExtra['id']);
        $sth2->execute();
        $check = $sth2->fetchObject();
    ...
    $sql = "UPDATE extra SET ".implode(',', $sets)." WHERE extra.id = :id";
    $sth = $this->app->db->prepare($sql);
    $sth->bindValue(':id',$inputExtra['id']);
    foreach ($inputExtra as $key => $value) {
        $sth->bindValue(':'.$key,$value);
    }
    $sth->execute();
...
function deleteExtra($request,$response,$args){
    ...
    $this->app->db->beginTransaction();
    try{
        $sth2 = $this->app->db->prepare("DELETE FROM extra WHERE extra.id = :extra");
        $sth2->bindParam(":extra", $inputExtra['id']);
        $sth2->execute();
...
function getExtra($request,$response,$args){
    //TODO TOKEN HERE
    $sth = $this->app->db->prepare("
        SELECT * FROM extra WHERE extra.id = :extraId;");
...
function addSize($request, $response){
    ...
    $sth2 = $this->app->db->prepare("SELECT size.id FROM size WHERE size.name = :name and size.product_id = :productId");
    ...
    try{
        ...
        $sth2 = $this->app->db->prepare("UPDATE product set product.multi_stock_quantity = product.multi_stock_quantity + :quantity WHERE product.id = :productId");
...
function updateSize($request,$response,$args){
    ...
    $sth4 = $this->app->db->prepare("SELECT size.stock_quantity FROM size WHERE size.id = :sizeId and size.product_id = :productId");
    $sth4->bindParam("sizeId", $inputSize['id']);
    ...
    $sql = "UPDATE size SET ".implode(',', $sets)." WHERE size.product_id = :productId and size.id = :sizeId";
    $sth = $this->app->db->prepare($sql);
    $sth->bindValue(':productId',$inputSize['product_id']);
    $sth->bindValue(':sizeId',$inputSize['id']);
    ...
    if($product->stock_quantity < $inputSize['stock_quantity']){
        ...
        $sql = "UPDATE product set product.multi_stock_quantity = product.multi_stock_quantity + :psStock where product.id = :productId";
    }else{
...
function deleteSize($request,$response,$args){
    ...
    try{
        ...
        $sth = $this->app->db->prepare("DELETE FROM size where size.id = :sizeId and size.product_id = :productId");
        $sth->bindParam(":sizeId", $inputSize['id']);
        $sth->bindParam(":productId", $inputSize['product_id']);
        $sth->execute();

        $sth1 = $this->app->db->prepare("UPDATE product set product.multi_stock_quantity = product.multi_stock_quantity - :psStock where product.id = :productId");
        $sth1->bindParam(":productId", $inputSize['product_id']);
...
function decrementProductStock($products){
    foreach($products as $value){
        ...
        $sthStock = $this->app->db->prepare("SELECT product.is_stock, product.is_multi_stock FROM product WHERE product.id = :item");
        $sthStock->bindValue(':item',$value['id']);
        $sthStock->execute();
        $product = $sthStock->fetchObject();

        if ($product->is_stock == 1) {
            $sthStock = $this->app->db->prepare("UPDATE product SET product.stock_quantity = (product.stock_quantity - :quantity) where product.id = :item");
            $sthStock ->bindValue(':quantity',$value['quantity']);
            $sthStock ->bindValue(':item',$value['id']);
            $sthStock->execute();
        }else if($product->is_multi_stock == 1){
            $sql = "UPDATE size SET size.stock_quantity = size.stock_quantity - :quantity WHERE size.product_id = :productId and size.id = :sizeId";
            $sth = $this->app->db->prepare($sql);
            $sth->bindValue(':productId',$value["id"]);
            $sth->bindValue(':sizeId',$value["size_id"]);
            $sth->bindValue(':quantity',$value["quantity"]);
            $sth->execute();

            $sth1 = $this->app->db->prepare("UPDATE product set product.multi_stock_quantity = product.multi_stock_quantity - :quantity where product.id = :productId");
            $sth1->bindParam(":productId", $value["id"]);
...
function updateProductPosition($request, $response){
    ...
    foreach ($inputProduct as $key => $value) {
        $sql = "UPDATE product set product.position = :productPosition where product.id = :productId";
        $sth = $this->app->db->prepare($sql);
...
function updateServicePosition($request, $response){
    ...
    foreach ($inputService as $key => $value) {
        $sql = "UPDATE service set service.position = :servicePosition where service.id = :serviceId";
        $sth = $this->app->db->prepare($sql);
...
function updateProductCollectionPosition($request, $response){
    ...
    foreach ($inputCollection as $key => $value) {
        ...
        $sth->bindValue(':productId',$value["id"]);
        $sth->bindValue(':collectionId',$value["id"]);
        $sth->execute();
    }
...
function reminderSchedulingDay(){
    ...
    $this->app->db->beginTransaction();
    try{
        $sql = "SELECT scheduling.id, scheduling.user_id, scheduling.company_name, scheduling.user_name FROM scheduling
        WHERE TIMESTAMPDIFF(HOUR, :hoje, scheduling.start_time) <= 24 AND scheduling.status = :status and scheduling.reminder_count = :reminderStatus and scheduling.source != :companyNuppin";

        $sth = $this->app->db->prepare($sql);
        $sth->bindValue(":hoje",date('Y-m-d H:i:s'));
        $sth->bindValue(":status",'accepted');
        $sth->bindValue(":reminderStatus",0);
        $sth->bindValue(":companyNuppin","company_nuppin");
        $sth->execute();
        $data = $sth->fetchAll();
        ...
        if(sizeof($data)>0){
            foreach ($data as $scheduling) {
                ...
                $sql3 = "UPDATE scheduling set scheduling.reminder_count = 1 where scheduling.id = :schedulingId";
                $sth3 = $this->app->db->prepare($sql3);
                $sth3->bindValue(':schedulingId',$scheduling['id']);
                $sth3->execute();           
            }
        }
        $this->app->db->commit();
...
function reminderSchedulingHour(){
    ...
    try{
        $sql = "SELECT scheduling.id, scheduling.user_id, scheduling.company_name, scheduling.user_name FROM scheduling
        WHERE TIMESTAMPDIFF(HOUR, :hoje, scheduling.start_time) <= 1 AND scheduling.status = :status and scheduling.reminder_count = :reminderStatus and scheduling.source = :companyNuppin";
        ...
        if(sizeof($data)>0){
            foreach ($data as $scheduling) {
                ...
                $sql3 = "UPDATE scheduling set scheduling.reminder_count = 2 where scheduling.id = :schedulingId";
                $sth3 = $this->app->db->prepare($sql3);
                $sth3->bindValue(':schedulingId',$scheduling['id']);
                $sth3->execute();           
            }
...
try{
    $sth = $this->app->db->prepare("
        ...
        SELECT COUNT(orders.id) AS orders_count FROM orders where DATE(orders.created_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT(scheduling.id) AS scheduling_count FROM scheduling where DATE(scheduling.created_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT( product.id) AS product_count FROM product where DATE(product.created_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT(invoice.id) AS invoice_count FROM invoice where DATE(invoice.created_date) = date(CURDATE() - INTERVAL 1 DAY);

        SELECT COUNT(invoice.id) AS invoice_paid_count,
        ...
        SELECT COUNT(invoice.id) AS invoice_canceled_count,
        COALESCE(SUM(invoice.total_amount),0) AS total_amount,
        COALESCE(SUM(invoice.subtotal_amount),0) AS subtotal_amount, 
        ...
        SELECT COUNT(company.id) AS company_count FROM company where DATE(company.created_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT(coupon.id) AS coupon_count FROM coupon where DATE(coupon.created_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT(employee.id) AS employee_count FROM employee where DATE(employee.hired_date) = date(CURDATE() - INTERVAL 1 DAY);
        SELECT COUNT(users.id) AS users_count FROM users where DATE(users.created_date) = date(CURDATE() - INTERVAL 1 DAY);");
    $sth->execute();
...
function viewCounter($companyId){
        ...
        if($view){
            ...
            $sth2 = $this->app->db->prepare("INSERT into view_count(id, company_id, count, created_date) values(:id,:companyId,:count,:created_date)");
...
function notificationRegister($tokenNotification, $userId, $source){
    $this->app->db->beginTransaction();
        ...
        $sth = $this->app->db->prepare("DELETE from notification where notification.id = :notificationId");
        $sth->bindValue(":notificationId",$tokenNotification);
        $sth->execute(); 

        $sth1 = $this->app->db->prepare("SELECT * FROM token where token.user_id = :userId and not exists (select * from notification where notification.refresh_token = token.id) order by token.created_date desc limit 1");
        $sth1->bindValue(":userId", $userId);
        $sth1->execute();
        $refreshToken = $sth1->fetchObject();

        $sth2 = $this->app->db->prepare("INSERT INTO notification (id, user_id, source, refresh_token) values(:notificationId, :userId, :source, :refreshToken)");
        $sth2->bindValue(":userId",$userId);
        $sth2->bindValue(":notificationId",$tokenNotification);
        $sth2->bindValue(":source",$source);
        $sth2->bindValue(":refreshToken",$refreshToken->id);
        $sth2->execute();   

        $sth4 = $this->app->db->prepare("DELETE from temp_notification where temp_notification.id = :notificationId");
...
function notificationTempToken($tokenNotification, $source){
    $sth1 = $this->app->db->prepare("SELECT * FROM temp_notification where temp_notification.id = :notificationId");
    $sth1->bindValue(":notificationId", $tokenNotification);
    $sth1->execute();
    $notification = $sth1->fetchObject();

    if(!$notification){
        $sth = $this->app->db->prepare("INSERT into temp_notification (id, source) values(:notificationId, :source)");
        $sth->bindValue(":notificationId",$tokenNotification);
        $sth->bindValue(":source",$source);
        $sth->execute(); 
...
function getListAnalises($request,$response,$args){
    ...
    if (!($args['companyId'] == $token["comp"] || $token["id"] == "ebfecbf08")) {
        return $response->withJson(false)->withStatus(403);
    }
    ...
    $sth = $this->app->db->prepare("
        ...
        where vm.validation_id = validation.id");
    $sth->bindParam("companyId", $args['companyId']);
    $sth->execute();
    $validation = $sth->fetchObject();
    $sth->nextRowset();
...
function getCompaniesValidation($request, $response, $args){
    ...
    if (!($token["id"] == "ebfecbf08")) {
        return $response->withJson(false)->withStatus(403);
    }

    $sth = $this->app->db->prepare("
        SELECT company.*, validation.id as validation_id,
        cc.name as category_name,
        sc.name as subcategory_name
        FROM company
        join category_company as cc on cc.id = company.category_company_id
        join subcategory_company as sc on sc.id = company.subcategory_company_id
        join validation on validation.company_id = company.id and validation.status = 'pending'
    ");
    $sth->execute();
    $company = $sth->fetchAll();
...
function updateValidationCompany($request, $response){
    ...
    $this->app->db->beginTransaction();
    try{
        $sth1 = $this->app->db->prepare("UPDATE validation set validation.status = :status WHERE validation.id = :validationId");
        $sth1->bindValue(':status',$validation['status']);
        $sth1->bindValue(':validationId',$validation['id']);
        $sth1->execute();

        if ($validation['status'] == "accepted") {
            $sth2 = $this->app->db->prepare("UPDATE company set company.visibility = 1 WHERE company.id = :companyId");
...
function deleteValidationMessage($request, $response){
    ...
    $this->app->db->beginTransaction();
    try{
        $sth1 = $this->app->db->prepare("DELETE from validation_message AS vm WHERE vm.id = :validationMessageId");
        $sth1->bindValue(':validationMessageId',$validationMessage['id']);
        $sth1->execute();
        $this->app->db->commit();
        return $response->withJson(true);
    }catch(\Throwable $e){
...
function logicaPrecoDelivery($cartCompany, $type){
    ...
    $cartCompany->delivery_amount = number_format((float)$this->roudHalf($cartCompany->delivery_amount), 2, '.', '');
    $cartCompany->total_amount = number_format((float)(($cartCompany->subtotal_amount - $cartCompany->discount_amount) + $cartCompany->delivery_amount), 2, '.', '');
}
...
function couponCartInfo($cartCompany, $couponId){
    $sth = $this->app->db->prepare("
        SELECT coupon.*
        from coupon
        join coupon_users on coupon_users.coupon_id = coupon.id
        where coupon_users.user_id = :userId and coupon.id = :couponId and coupon.company_id = :companyId and coupon_users.order_id is null and coupon.due_date > :data");
    ...
    if ($coupon) {
        ...
        if ($coupon->discount_type == 1) {
            ...
            $cartCompany->discount_amount =  number_format((float)$this->roudHalf((($coupon->value / 100) * $cartCompany->subtotal_amount)), 2, '.', '');
        }
    }
}
...
function updateCartInfo($request, $response) {
    ...
    try{
        $sth = $this->app->db->prepare("UPDATE cart_info as info SET info.payment_method = :paymentMethod, info.type = :type where info.user_id = :userId and info.company_id = :companyId");
...
function updateCartCouponInfo($request, $response) {
    ...
    $sth = $this->app->db->prepare("
        SELECT
        (sum((product.price + (SELECT COALESCE(sum(size.price),0) FROM size where size.id = cart.size_id) + (SELECT COALESCE(SUM(cart_extra.quantity * extra.price),0) FROM cart_extra join extra on cart_extra.extra_id = extra.id where cart_extra.cart_id = cart.id))* cart.quantity)) subtotal_amount
        FROM cart
        join product on cart.product_id = product.id
        where cart.user_id = :userId and cart.source = :source;
    ...
    $sth->bindValue(":data", date("Y-m-d H:i:s"));  
    $sth->bindValue(":source", $inputCartInfo['source']);
    $sth->execute();
    $checkerCart = $sth->fetchObject();
    $sth->nextRowset();
    $checkerCoupon = $sth->fetchObject();
  
    if ($checkerCoupon || $inputCartInfo["coupon_id"] === "null") {
...
function verificarPedido($safeOrder){
    ...
    if (!$safeOrder->payment_method) {
        $error = array();
        $error['error_code'] = "001";
        $error['error_message'] = "Metodo de pagamento não escolhido";
        $arrayJson = array("error" => $error, "order"=> false);
        return $arrayJson;
    }

    foreach($safeOrder->product as $valueItem){
        $sthStock = $this->app->db->prepare("SELECT * FROM product WHERE product.id = :item");
        $sthStock ->bindValue(':item',$valueItem['product_id']);
        $sthStock->execute();
        $product = $sthStock->fetchObject();

        if ($product->is_stock == 1) {
            $sthStock = $this->app->db->prepare("SELECT product.id FROM product WHERE product.is_stock = 1 and product.stock_quantity >= (select sum(cart.quantity) from cart where cart.product_id = :item and cart.user_id = :userId) and product.id = :item");
            $sthStock ->bindValue(':item',$valueItem['product_id']);
            $sthStock ->bindValue(':userId',$safeOrder->user_id);
            $sthStock->execute();
            $stock = $sthStock->fetchObject();      
        }else if ($product->is_multi_stock == 1){
            $sthStock = $this->app->db->prepare("SELECT size.product_id from size where size.id = :sizeId AND size.product_id = :productId AND size.stock_quantity >= (select sum(cart.quantity) from cart where cart.product_id = :productId and cart.user_id = :userId and cart.size_id = :sizeId)");
    ...
    if (!($safeOrder->source == "nuppin_company")) {
        if($safeOrder->coupon_id){
            $sqlCoupon = "SELECT coupon.min_purchase FROM coupon WHERE coupon.id = :couponId;";
...
function adicionarProdutosDoPedido($safeOrder){
    ...
    foreach($safeOrder as $orderKey => $orderValue){
        if($orderKey != "product"){
            if($orderKey == "id"){
                $safeOrder->id = $this->uniqidReal(6);
                $sth ->bindValue(":".$orderKey,$safeOrder->id);
            }else{
    ...
    foreach($safeOrder->product as $itemKey => $itemValues){
        ...
        foreach($itemValues as $itemKey => $itemValue){
            if($itemKey != "extra"){
                if ($itemKey == "order_id") {
                    $sth2->bindValue(":".$itemKey,$safeOrder->id);
                }else if ($itemKey == "id") {
        ...
        foreach($extras as $extrasKey => $arrayExtras){
            ...
            foreach($arrayExtras as $ekey => $evalue){
                if ($ekey == "order_id") {
                    $sth4->bindValue(":".$ekey,$safeOrder->id);
    ...
    $sth4 = $this->app->db->prepare("DELETE from cart_info where cart_info.user_id = :id and cart_info.source = :source");
    $sth4->bindValue(":id",$safeOrder->user_id);
    $sth4->bindValue(":source",$safeOrder->source);
    $sth4->execute();
}
...
function inserirNoCarrinho($inputCart){
    if (!($inputCart["source"] == "nuppin_company")) {
        ...
        if(!$info){
            ...
            $sth1 = $this->app->db->prepare("
                SELECT company.is_delivery, company.is_local FROM company where company.id = :companyId;
                SELECT users.full_name from users where users.id = :userId");
        ...
        foreach ($inputCart as $key => $value) {
            if ($key != "extra") {
                if($key == "id"){
                    $inputCart["id"] = $this->uniqidReal(9);
                    $sth2->bindValue(":".$key, $inputCart["id"]);
                }else{
                    $sth2->bindValue(':'.$key,$value);
                }
            }
...
function orderSafeComplementMethods($order){
    ...
    $sth = $this->app->db->prepare("SELECT * FROM company where company.id = :companyId");
...
function getCartCompany($request,$response,$args){
    ...
    $sth = $this->app->db->prepare(
       ...
       (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:longitude,:latitude))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(mobile.longitude, mobile.latitude), POINT(:longitude,:latitude))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:longitude,:latitude))/1000)end))end) as distance,
        ...
        from company
        left join cart on cart.company_id = company.id AND cart.user_id = :userId and cart.source = 'nuppin_company'
        ...
        from cart
        join product on cart.product_id = product.id
        left join cart_extra on cart.id = cart_extra.cart_id
        left join extra on cart_extra.extra_id = extra.id
    ...
    if($product != false){
        if (sizeof($extras)>0) {
            foreach ($product as &$p) {
                foreach ($extras as $e) {
                    if ($p["product_id"] == $e["product_id"] && $p["id"] == $e["cart_id"]) {
                        $p["extra"][] = $e;
                    }
                }
            }
...
function addOrderCompany($request,$response){
    ...
    try{
        ...
        if ($arrayError) {
            return $response->withJson($arrayError)->withStatus(400);
        }
...
function safeWaytoPrepareOrderCompany($inputOrder){
    $sth = $this->app->db->prepare(
        ...
        (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:userLongitude,:userLatitude))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(mobile.longitude, mobile.latitude), POINT(:userLongitude,:userLatitude))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(:userLongitude,:userLatitude))/1000)end))end) as distance,
        cart.source
        from cart
        join product on cart.product_id = product.id
        join company on company.id = cart.company_id
        ...
        ce.name,
        ce.quantity,
        (SELECT extra.price FROM extra where extra.id = ce.extra_id) as unit_amount,
        (SELECT sum(ce.quantity * extra.price) FROM extra where extra.id = ce.extra_id) as total_amount
    ...
    if($company != false){
        ...
        if (sizeof($extras)>0) {
            ...
            foreach ($product as $p) {
                foreach ($extras as $e) {
                    if ($p["product_id"] == $e["product_id"] && $p["id"] == $e["order_item_id"]) {
                        $p["extra"][] = $e;
                    }
                }
                array_push($productExtras, $p);
...
function getCartUser($request,$response,$args){
    ...
    $sth = $this->app->db->prepare(
        ...
        company.latitude,
        company.longitude,
        address.full_address as user_address,
        address.latitude as user_latitude,
        address.longitude as user_longitude,
        (sum((product.price + (SELECT COALESCE(sum(size.price),0) FROM size where size.id = cart.size_id) + (SELECT COALESCE(SUM(cart_extra.quantity * extra.price),0) FROM cart_extra join extra on cart_extra.extra_id = extra.id where cart_extra.cart_id = cart.id))* cart.quantity)) subtotal_amount,

        (case when company.model_type = 'fixed' then (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(address.longitude,address.latitude))/1000) else ((case when IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL),1,0) = 1 then (SELECT ST_Distance_Sphere(POINT(mobile.longitude, mobile.latitude), POINT(address.longitude,address.latitude))/1000 FROM mobile where mobile.company_id = company.id and mobile.end_date IS NULL) else (ST_Distance_Sphere(POINT(company.longitude, company.latitude), POINT(address.longitude,address.latitude))/1000)end))end) as distance,
        ...
        (case when company.model_type = 'fixed' then
            ...
            (
            IF(EXISTS(SELECT mobile.company_id FROM mobile where mobile.company_id = company.id and(mobile.company_id = company.id and mobile.end_date IS NULL)),1,0)
            )
        end) AS is_online
        from cart
        join product on cart.product_id = product.id
        ...
        where cart.user_id = :id and cart.source = :source
        group by cart.company_id;
        ...
        SELECT coupon.*
        from coupon_users
        join coupon on coupon.id = coupon_users.coupon_id
        join cart on cart.user_id = :id and cart.source = :source
        where coupon_users.user_id = :id and
        coupon.id = coupon_users.coupon_id and 
    ...
    if($args['hasPayments']){
        $sth = $this->app->db->prepare("SELECT * from payment where EXISTS (select payco.company_id from payment_company as payco where payco.company_id = :companyId and payment.id = payco.payment_id)");
        $sth->bindParam(":companyId", $args['hasPayments']);            
        $sth->execute();
...
function safeWaytoPrepareOrder($order){
    ...
    $sth->bindParam(":source", $order["source"]);     
    $sth->bindValue(":selected", "1");                    
    $sth->execute();
    $company = $sth->fetchObject();
    $sth->nextRowset();
...
function addOrder($request,$response){
    ...
    try{
        ...
        $this->send_notification($safeOrder->company_id,"Pedido: ".strtoupper($safeOrder->id)." - Clique e atualize o status", "Novo Pedido", $this->app->db, "all_company");
        ...
        $error = array();
        $error['error_code'] = "001";
        $error['error_message'] = "Houve um erro ao finalizar seu pedido";
        $arrayJson = array("error" => $error,"order" => false);
        return $response->withJson($safeOrder);
    }
}
...
function addIndication($request, $response){
    $inputIndication = $request->getParsedBody();
    $token = $request->getAttribute("token");
    
    if (!($inputIndication['user_id'] == $token["id"])) {
        return $response->withJson(false)->withStatus(403);
    }

    $sthCheck = $this->app->db->prepare("SELECT * FROM indication WHERE indication.reference = :reference");
    $sthCheck ->bindValue(':reference',$inputIndication['reference']);
    $sthCheck->execute();
    $indication = $sthCheck->fetchObject();

    if(!$indication){
        $keys = array_keys($inputIndication);

        $sth = $this->app->db->prepare("INSERT INTO indication (id,status,".implode(',', $keys).") VALUES (:id,:status,:".implode(",:", $keys).")");
        $sth->bindValue(':id',$this->uniqidReal(9));
        $sth->bindValue(':status',"pending");
        foreach ($inputIndication as $key => $value) {
                $sth->bindValue(':'.$key,$value);
        }
        $sth->execute();
        return $response->withStatus(200);
...
function hasIndication($userId, $companyId){
    $sth = $this->app->db->prepare("SELECT users.email, users.phone_number FROM users WHERE users.id = :userId");
    $sth->bindValue(':userId',$userId);
    $sth->execute();
    $user = $sth->fetchObject();

    $checkReference = $this->app->db->prepare("SELECT * FROM indication WHERE indication.reference = :email or indication.reference = :phoneNumber");
    $checkReference->bindValue(':email',$user->email);
    $checkReference->bindValue(':phoneNumber',$user->phone_number);
    $checkReference->execute();
    $indication = $checkReference->fetchObject();

    if($indication){
        $sth = $this->app->db->prepare("INSERT INTO affiliated_company (id, company_id, user_id) VALUES (:id, :companyId, :userId)");
        $sth->bindValue(':id'.$key,$this->uniqidReal(9));
        $sth->bindValue(':companyId', $companyId);
        $sth->bindValue(':userId',$indication->user_id);
        $sth->execute();
        $sth1 = $this->app->db->prepare("DELETE FROM indication where indication.id = :indicationId");
        $sth1->bindValue(':indicationId',$indication->id);
        $sth1->execute();
    }
}

function addTransaction($amount, $userId, $status, $description){
        $sth = $this->app->db->prepare("INSERT INTO transaction (id, user_id, amount, status, description) VALUES (:id, :userId, :amount, :status, :description)");
        $sth->bindValue(':id',$this->uniqidReal(9));
        $sth->bindValue(':userId',$userId);
        ...
        $sth1 = $this->app->db->prepare("UPDATE users set users.balance = (users.balance + :newBalance) where users.id = :userId");
        $sth1->bindValue(':userId',$userId);
        $sth1->bindValue(':newBalance',$amount);
        $sth1->execute();
}

function cashInAffiliate($companyId, $amount, $fee){
 
    $sth = $this->app->db->prepare("SELECT * FROM affiliated_company as affiliated WHERE affiliated.company_id = :companyId");
    $sth->bindValue(':companyId',$companyId);
    $sth->execute();
    $affiliate = $sth->fetchObject();

    if($affiliate){
        $this->addTransaction(((($amount-$fee)*0.50)/100), $affiliate->user_id, "concluded", "Comissão");
    }
}

function cashOutUser($request,$response,$args){
    $inputUser = $request->getParsedBody();
    $token = $request->getAttribute("token");
    ...
    try{
        $sth = $this->app->db->prepare(
        "SELECT users.balance as amount from users where users.id = :userId");

        $sth->bindParam(":userId", $inputUser["id"]);
        $sth->execute();
        $balance = $sth->fetchObject();

        if ($balance->amount < 50) {
            return $response->withJson("Saldo abaixo do mínimo necessário para saque")->withStatus(400);
        }

        //$fee = -5.00;
        //$this->addTransaction($fee, $inputUser["id"], "pending", "Taxa de saque");
        //$this->addTransaction(-$balance->amount-$fee, $inputUser["id"], "pending", "Saque");

        $this->addTransaction(-$balance->amount, $inputUser["id"], "pending", "Saque");
...
function getTransaction($request,$response,$args){
    $token = $request->getAttribute("token");
    ...
    $sth = $this->app->db->prepare(
    "SELECT transaction.amount, transaction.description, transaction.status, transaction.id, DATE_FORMAT(transaction.created_date, '%d/%m') as created_date  from transaction where transaction.user_id = :userId order by transaction.created_date desc;
    SELECT users.balance as amount from users where users.id = :userId");
...
function getIndication($request,$response,$args){
    $token = $request->getAttribute("token");
    ...
    $sth = $this->app->db->prepare(
    "SELECT indication.id, indication.reference, DATE_FORMAT(indication.created_date, '%d/%m') as created_date from indication where indication.user_id = :userId order by indication.created_date desc");

    $sth->bindParam(":userId", $args['userId']);
    $sth->execute();
    $indication = $sth->fetchAll();
    return $response->withJson($indication);
}

function getUserAffiliatedCompany($request,$response,$args){
    $token = $request->getAttribute("token");
    ...
    $sth = $this->app->db->prepare("SELECT DISTINCT(affiliated.id), DATE_FORMAT(affiliated.created_date, '%d/%m') as created_date,
    (select users.email from employee join users on users.id = employee.user_id
    where employee.company_id = affiliated.company_id and employee.role = 'owner') as reference,
    invoice.status
    from affiliated_company as affiliated
    LEFT JOIN invoice ON invoice.id = (select invoice.id from invoice WHERE invoice.company_id = affiliated.company_id order by invoice.created_date desc LIMIT 1)
    where affiliated.user_id = :userId ORDER BY invoice.created_date
    ");
    $sth->bindParam(":userId", $args['userId']);
    $sth->execute();
    $indication = $sth->fetchAll();
